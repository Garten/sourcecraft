 ...
 *
 */
public class SupplyTf2 extends ConvertAction {
...
//		context.addSolidEntity(new FuncRegenerate(this.createArea(p, end), Tf2Team.ANY, createdProp));
		context.markAsConverted(p, end);
	}

	private Solid createArea(ConverterContext context, Position p, Position end) {
		return context.createCuboid(p, end.getOffset(0, 2, 0), null);
	}

	private void moveToMiddle(ConverterContext context, Position p, Position end) {
